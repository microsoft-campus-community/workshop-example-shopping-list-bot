 ...
 * 
 * Precondition: shoppingListID and itemID must not be null, undefined or empty string.
 * 
 ...
 * @param context.bindingData.shoppingListID uniquely identifies the shopping list from which an {@link Item} should be removed.
 ...
 * @returns status ```200``` if the item could be removed from the shopping list belonging to the given shoppingListID.
 *      Or http status ```404``` if the shopping list could not be found.
 *      Or http status ```400``` on invalid input.
 * 
 * [DELETE] http://<FUNCTION_URL>/api/RemoveItemByIDFunction/{shoppingListID}/{itemID}
 */
const httpTrigger: AzureFunction = async function (context: Context): Promise<void> {
    const shoppingListID = context.bindingData.shoppingListID;
    const itemID = context.bindingData.itemID;
    if (!shoppingListID || shoppingListID === '' || !itemID || itemID === '') {
        context.res = {
            status: 400,
            body: {
