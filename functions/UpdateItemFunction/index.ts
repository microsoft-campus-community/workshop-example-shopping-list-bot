 ...
 * 
 * Precondition: conversationId and itemID must not be null, undefined or an empty string.
 ...
 * @param context.bindingData.conversationID uniquely identifies the shopping list which contains the {@link Item} to update.
 ...
 * 
 * [PATCH] http://localhost:7071/api/UpdateItem/{conversationID}/{itemID}
 */
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    const conversationID = context.bindingData.conversationID;
    const itemID = context.bindingData.itemID;

    if (!conversationID || conversationID === '' || !itemID || itemID === '' || !req.body || req.body.positionInShoppingList !== undefined || req.body.id !== undefined) {
        context.res = {
            status: 400,
            body: {
