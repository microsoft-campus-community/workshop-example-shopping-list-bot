...
/**
 * Http endpoint to add an {@link Item} passed in the request to the shopping list belonging to a certain shoppingListID.
 * 
 * Precondition: Item and item's name must not be undefined and item's name must not be an empty string. ShoppingListID must not be undefined or an empty string.
 *
 ...
 * @param context.bindingData.shoppingListID item is added to the shopping list belonging to this ID.
 ...
 * 
 * [POST] http://<FUNCTION_URL>/api/AddItemFunction/{shoppingListID}
 */
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    const shoppingListID = context.bindingData.shoppingListID;
    if (!shoppingListID || shoppingListID === '' || !req.body || !req.body.itemName) {
        console.dir("[DEBUG] ITEM: " + req.body);
        console.dir("[DEBUG] ID: " + shoppingListID);
        context.res = {
            status: 400,
            body: {
